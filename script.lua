...
if getgenv().Nousigi then 
	if game.CoreGui:FindFirstChild("Banana Hub GUI") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name,  "Banana Hub") then
				v:Destroy()
			end
		end
	end
...
local T1UIColor = {
    ...
    ["Title Text Color"] = Color3.fromRGB(255, 255, 0),
    ...
    ["Dropdown Selected Color"] = Color3.fromRGB(255, 255, 0),
    ["Dropdown Selected Check Color"] = Color3.fromRGB(255, 255, 0),
...
Library_Function.Gui.Name = 'Banana Hub GUI'
...
Library_Function.NotiGui.Name = 'Banana Hub Notification'

Library_Function.HideGui = Instance.new('ScreenGui')
Library_Function.HideGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Library_Function.HideGui.Name = 'Banana Hub'


local UIS = game:GetService("UserInputService")

if game.CoreGui:FindFirstChild("NazuXWindowsToggleUltimate") then
    game.CoreGui.NazuXWindowsToggleUltimate:Destroy()
end
...
mainButton.Position = UDim2.new(0,15,1,-20)
...
icon.Image = "rbxassetid://5009915795"
...
Library.ToggleUI = function()
    ...
    local iconScaleTween = TweenService:Create(ImageLabel, defaultTweenInfo, {
        Size = isToggled and UDim2.new(0, 47, 0, 47) or UDim2.new(0, 40, 0, 40)
    })
...
local libCreateNoti = function(Setting)
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,80,80") .. ")\">" .. tostring(TitleNameMain or "Banana Hub") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title or "Banana Hub"
   ...
   TextLabelMain.Text = "<font color=\"rgb(220, 200, 80)\">" .. tostring(TitleNameMain or "Banana Hub") .. "</font> <font color=\"rgb(255,255,255)\">" .. tostring(getgenv().MainDesc or "") .. "</font>"
    ...
    ButtonClick()
			...
			function sectionFunction:AddButton(Setting, Callback)
				...
				ClickArea_1.BackgroundColor3 = Color3.fromRGB(220, 200, 80)
